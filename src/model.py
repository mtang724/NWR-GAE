from src.layers import MLP, MLP_generator
import torch
...
class GNNStructEncoder(nn.Module):
    def __init__(self, node_num, in_dim, hidden_dim, out_dim, layer_num, sample_size, device, neighbor_num_list, GNN_name="GIN"):
    ...
    def neighbor_decoder(self, gij, ground_truth_degree_matrix, neighbor_num_list, temp, g, h, neighbor_dict, device, test, l4):
        ...
        for _ in range(3):
            ...
            for i1, embedding in enumerate(gij):
                ...
                # std_z = self.mlp_m.sample().to(device)
                # var = self.mlp_gaussian_mean + self.mlp_gaussian_log_sigma.exp() * std_z
                # print("sample embedding", embedding.shape)
                # neighbors = self.neighbor_generator(var, device)
                #print("neighbor", neighbors.shape)
                generated_neighbors = []
                ...
                nhij = zij @ var
                #print("nij", nhij.shape)
                generated_neighbors = nhij.tolist()
                # generated_neighbors = neighbors.tolist()
    ...
    def forward(self, g, h, ground_truth_degree_matrix, neighbor_dict, neighbor_num_list, in_dim, temp, test, device):
        ...
        # print(time.time() - start_time)
        # start_time = time.time()
        loss, hij = self.neighbor_decoder(gij, ground_truth_degree_matrix, neighbor_num_list, temp, g, h, neighbor_dict, device, test, l4)
        # print("decoder time:")
        # print(time.time() - start_time)
        return loss, hij